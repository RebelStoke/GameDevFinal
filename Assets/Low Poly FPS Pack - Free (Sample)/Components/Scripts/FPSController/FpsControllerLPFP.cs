...
#pragma warning restore 649
        ...
        private void OnCollisionEnter(Collision collision) {
            ...
            {
                playerManager.GetComponent<PlayerManager>().GameOver(false);
            }
        }
        ...
        private void Jump()
        {
            ...
            int multiplier = 1;
            _isGrounded = false;
            if (playerManager.GetComponent<PlayerManager>().isHighJump == 1)
            multiplier = 2;
            _rigidbody.AddForce(Vector3.up * jumpForce * multiplier, ForceMode.Impulse);
        }

        private void Dash() {
            if (playerManager.GetComponent<PlayerManager>().isDash == 0 | !input.Dash)
                return;
            var direction = new Vector3(input.Move, 0f, input.Strafe).normalized;
            _rigidbody.AddForce(transform.TransformDirection(direction) * 2000f, ForceMode.Impulse);
        }
